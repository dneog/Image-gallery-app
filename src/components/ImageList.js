...
const ImageList = ({images}) => {
 ...
 const currentPageUrl = window.location.href
  ...
  const handleSaveLater = async (item)=> {
    if (isLoggedIn) {
      const docRef = await addDoc(collection(db, "favourate"), {
        userID,
        item,
        createdAt: Timestamp.now().toDate()
    
        });
      toast.success('Added To Favourate Successfully.')
    } else {
     ...
     <div className={styles.imgContainer}>
        {images.map((item)=> (
          <div className={styles.containerB}> 
            <img className={styles.imgs} src={item.webformatURL} onClick={()=> handleData(item)}  alt="images" />

          <div className={styles.wrapper}></div>
          <RiFolderAddLine className={styles.collection} onClick={()=> handleSaveLater(item)} />
